...
### load data
setwd('/Users/meali/Downloads/')

load("nci60_bin_2609.RData")
rownames(Y) = drug_names
Y = t(Y)

#select X 
rownames(XZ_bin) = colnames(XZ)
x = XZ_bin
x[x == -1] = 0
...
############# run lapatinib
ctype = read.table("NCI60_celltype.csv",sep=";",header=T)
ctype[,1] = gsub(ctype[,1],pattern="NCI-",replacement = "")
ctype[,1] = gsub(ctype[,1],pattern="-",replacement = "")
ctype[,1] = gsub(ctype[,1],pattern=" ",replacement = "")
ctype[,1] = gsub(ctype[,1],pattern="/",replacement = "")
ctype[,1] = gsub(ctype[,1],pattern="(TB)",replacement = "",fixed=T)
ctype[,1] = gsub(ctype[,1],pattern="ATCC",replacement = "",fixed=T)
ct = as.character(ctype[match(colnames(XZ),ctype[,1]),2])
ct[ct == "Non-Small Cell Lung"] = "Lung"
